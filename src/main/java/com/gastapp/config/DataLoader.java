...
public class DataLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        List<Expense> gastos = List.of(
            Expense.builder().id(UUID.randomUUID()).monto(new BigDecimal("150.50")).descripcion("Almuerzo").fecha(LocalDate.now()).category(cat).user(user).build(),
            Expense.builder().id(UUID.randomUUID()).monto(new BigDecimal("320.00")).descripcion("Supermercado").fecha(LocalDate.now().minusDays(1)).category(cat).user(user).build(),
            Expense.builder().id(UUID.randomUUID()).monto(new BigDecimal("45.00")).descripcion("Café").fecha(LocalDate.now().minusDays(2)).category(cat).user(user).build()
        );
        expenseRepository.saveAll(gastos);
        log.info("Gastos de prueba creados: {}", gastos.size());
    }
}
